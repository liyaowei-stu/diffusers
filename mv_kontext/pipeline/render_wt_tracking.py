...
from mv_kontext.utils import define_rasterizer_renderer, get_points_from_predictions, get_depth_fill_from_predictions, np_depth_to_colormap, convert_opencv_to_pytorch3d_c2w, img_tensor_to_pil, get_mask_from_fragments, extract_correspondence_from_fragments, normalize_image_coordinates, visualize_paired_correspondence, get_batch_split_indices, visualize_i_view_with_keypoints, denormalize_image_coordinates, transfer_coords_list_from_padded_to_original
...
def main(
    ...
    for i, data in enumerate(tqdm(data_loader)):
        ...
        points_flat, colors_flat, conf_flat_list, depth_fill, recon_coords_packed_list = get_recon_points_and_depth(sample_recon_images, sample_recon_extrinsics, sample_recon_intrinsics, sample_recon_masks, sample_recon_depths, sample_recon_depth_confs, sample_recon_world_points, sample_recon_world_points_confs, args.use_point_map, args.max_points, args.conf_threshold, args.conf_threshold_value, args.apply_mask)


        ## recon_coords_packed_list: list of packed recon_coords, (M_i, 3)
        recon_coords_packed_list_normalized = normalize_image_coordinates(recon_coords_packed_list, height=h, width=w, align_corners=True)
        ...
        binary_mask = get_mask_from_fragments(fragments, use_gaussian_blur=True, use_binary_mask=True)


        recon_coords_packed_list_normalized_valid, render_coords_packed_list_normalized_valid, conf_packed_list_valid = extract_correspondence_from_fragments(fragments, recon_coords_packed_list_normalized, align_corners=True, recon_images=sample_recon_images[0], conf_flat_list=conf_flat_list, render_mask=binary_mask)
        ...
        for i, new_image in enumerate(new_images):
            ...
            ## save renders
            # 1. save renders images
            render_new_image_np = new_image.cpu().numpy()
            render_new_image_np = render_new_image_np.astype(np.uint8)
            render_new_image = Image.fromarray(render_new_image_np)
            save_path = f"{save_dir}/renders/rasterized/sample_idx_{sample_render_idxs[i].item()}.png"
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            render_new_image.save(save_path)

            # 2. save gt render images
            save_path = f"{save_dir}/renders/gt/sample_idx_{sample_render_idxs[i].item()}.png"
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            save_image(sample_render_ori_gt_images[i] / 2 + 0.5, save_path)

            # 3. save rasterized depth color
            save_path = f"{save_dir}/renders/rasterized_depth_color/sample_idx_{sample_render_idxs[i].item()}.png"
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            Image.fromarray(new_depth_color[i]).save(save_path)

            # 4. save rasterized depth
            save_path = f"{save_dir}/renders/rasterized_depth/sample_idx_{sample_render_idxs[i].item()}.png"
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            depth_img = (new_depth_normalized[i] * 255).astype(np.uint8)
            if depth_img.ndim == 2:
                pass  # already HxW
            elif depth_img.ndim == 3 and depth_img.shape[2] == 1:
                depth_img = np.squeeze(depth_img, axis=2)
            else:
                raise ValueError(f"Unexpected depth image shape: {depth_img.shape}")
            Image.fromarray(depth_img).save(save_path)

            # 5. save binary mask
            save_path = f"{save_dir}/renders/binary_mask/sample_idx_{sample_render_idxs[i].item()}.png"
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            Image.fromarray((binary_mask[i] * 255).astype(np.uint8)).save(save_path)
    
            # 6. save correspondence
            save_sub_dir = f"{save_dir}/renders/correspondence"
            os.makedirs(save_sub_dir, exist_ok=True)

            # recon_coords_packed_list_denormalized = denormalize_image_coordinates(recon_coords_packed_list_normalized, height=h, width=w, align_corners=True)
            # for view_idx in range(len(sample_recon_images[i])):
            #     visualize_i_view_with_keypoints(recon_coords_packed_list_denormalized[i], sample_recon_images[i], i=view_idx, output_prefix=f"recon_coords_bsz_{i}", output_dir=save_sub_dir)

            # recon_coords_packed_list_denormalized_valid = denormalize_image_coordinates(recon_coords_packed_list_normalized_valid, height=h, width=w, align_corners=True)
            # for view_idx in range(len(sample_recon_images[i])):
            #     visualize_i_view_with_keypoints(recon_coords_packed_list_denormalized_valid[i], sample_recon_images[i], i=view_idx, output_prefix=f"recon_coords_valid_bsz_{i}", output_dir=save_sub_dir)

            # recon_coords_packed_list_denormalized_valid_ori = transfer_coords_list_from_padded_to_original(recon_coords_packed_list_denormalized_valid, padded_h=h, padded_w=w, original_h=sample_recon_ori_gt_images.shape[-2], original_w=sample_recon_ori_gt_images.shape[-1], is_normalized=False)
            # for view_idx in range(len(sample_recon_images[i])):
            #     visualize_i_view_with_keypoints(recon_coords_packed_list_denormalized_valid_ori[i], (sample_recon_ori_gt_images[i]+1)/2, i=view_idx, output_prefix=f"recon_coords_valid_ori_bsz_{i}", output_dir=save_sub_dir)


            # visualize_paired_correspondence(recon_coords_packed_list_normalized_valid[i], render_coords_packed_list_normalized_valid[i], sample_recon_ori_gt_images[i], sample_render_ori_gt_images[i], output_dir=save_sub_dir, output_prefix=f"render_gt_rencon_correspondence_vis_bsz_{i}", max_points=100, denormalize_coords=True, padded_h=h, padded_w=w, conf_packed=conf_packed_list_valid[i], color="lime", lw=0.4, ps=4, a=1.0, sample_recon_extrinsics=sample_recon_extrinsics[i], sample_recon_intrinsics=sample_recon_intrinsics[i], sample_render_extrinsics=sample_render_extrinsic[i], sample_render_intrinsics=sample_render_intrinsic[i])

            render_new_image_normalized = torch.from_numpy((render_new_image_np/255)*2-1).permute(2, 0, 1)
            visualize_paired_correspondence(recon_coords_packed_list_normalized_valid[i], render_coords_packed_list_normalized_valid[i], sample_recon_ori_gt_images[i], render_new_image_normalized , output_dir=save_sub_dir, output_prefix=f"render_preds_recon_correspondence_vis_bsz_{i}", max_points=None, denormalize_coords=True, padded_h=h, padded_w=w, conf_packed=conf_packed_list_valid[i], color="lime", lw=0.4, ps=4, a=1.0,sample_recon_extrinsics=sample_recon_extrinsics[i], sample_render_extrinsics=sample_render_extrinsic[i], match_ratio_threshold=0.15, rotation_threshold=90, translation_threshold=1.0)
            ...
            dpt_normalized = depth_normalized[i].squeeze()
            ...
            for j in range(len(sample_recon_image)):
                ...
                os.makedirs(os.path.dirname(save_path), exist_ok=True)
                Image.fromarray(dpt_color[j]).save(save_path)

                # 9. save depth
                save_path = f"{save_dir}/recon/depth/sample_idx_{sample_recon_idx[j].item()}.png"
                os.makedirs(os.path.dirname(save_path), exist_ok=True)
                Image.fromarray((dpt_normalized[j] * 255).astype(np.uint8)).save(save_path)

            # 10. save points cloud
